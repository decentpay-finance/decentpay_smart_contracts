...
// ArtichainToken with Governance.
contract ArtichainToken is BEP20('Artichain Token', 'AIT') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterArt).
    ...
    constructor() public {
       _mint(msg.sender, 22650e18);  // mint 22.65k
    }
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "AIT::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "AIT::delegateBySig: invalid nonce");
        require(now <= expiry, "AIT::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "AIT::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying AITs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "AIT::_writeCheckpoint: block number exceeds 32 bits");
